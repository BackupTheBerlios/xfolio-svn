...
-->
<xsl:transform xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" version="1.1">
	...
	<!-- language extract from filename -->
...
-->
	<xsl:template name="dc:properties">
		...
		<!-- dates, get a revision desc ? -->
		<xsl:for-each select="/*/*[contains(local-name(), 'info')][1]//date">
			<dc:date xsi:type="http://www.w3.org/TR/NOTE-datetime">
				<xsl:value-of select="."/>
			</dc:date>
		</xsl:for-each>
		<!-- type -->
 ...
 -->
	<xsl:template match="author | editor">
		<xsl:choose>
			...
			<xsl:when test="surname | firstname">
		...
		<xsl:if test=".//affiliation">
			<xsl:text> (</xsl:text>
			<xsl:for-each select=".//affiliation[1]/*">
				<xsl:apply-templates select="."/>
	...
	<xsl:template match="personname" name="personname">
		<xsl:apply-templates select="surname"/>
		<xsl:if test="firstname">
			<xsl:text>, </xsl:text>
			<xsl:apply-templates select="firstname"/>
		</xsl:if>
		<xsl:if test="lineage|othername">
			<xsl:text>, </xsl:text>
